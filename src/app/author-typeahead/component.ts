  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => AuthorTypeaheadComponent),
      multi: true,
    },
  ],
})
export class AuthorTypeaheadComponent implements ControlValueAccessor {
  ...
  async save() {
    if (this.isNew && this._customSelected) {
      ...
      const data = await this.authorsService.create({
